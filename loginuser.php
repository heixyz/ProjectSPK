...
if (isset($_GET['login'])) {
    $tahun = $_GET['tahun'];
    $query = mysqli_query($conn, "SELECT * FROM user WHERE tahun = '$tahun'");
    if ($query->num_rows == 1) {
        $query = mysqli_query($conn, "SELECT * FROM user WHERE tahun = '$tahun'");
        if ($query->num_rows == 1) {
            $_SESSION['user'] = $row = $query->fetch_assoc();
            echo "<script>
    alert('Login Sukses');
    document.location.href = 'indexUser.php';
        ...
        echo "<script>
alert('Data tahun berikut masih belum di-Input');
        ...
        <div class="row w-100 mx-0">
            <div class="col-lg-4 mx-auto">
                <div class="auth-form-light text-left py-4 px-4 px-sm-5">
                    ...
                    <h6 class="font-weight-light">Masukkan data tahun film</h6>
                    <hr>
                    <form class="pt-1" method="get">
                        <div class="form-group">
                            <input type="text" name="tahun" class="form-control form-control-lg" placeholder="Tahun">
                        </div>
                        <div class="mt-1">
